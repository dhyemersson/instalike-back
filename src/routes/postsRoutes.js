...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        // Especifica a pasta onde os arquivos serão salvos
        cb(null, 'uploads'); 
    },
    filename: function (req, file, cb) {
        // Gera um sufixo único para evitar duplicação de nomes
        const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1e9);
        cb(null, uniqueSuffix + '-' + file.originalname); // Nome final do arquivo
    }
});
